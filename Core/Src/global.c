...
void command_parser_fsm(int value)
{
	...
	case 0:
		if(value == 33){
			state = 1;
			HAL_UART_Transmit(&huart2, (void *)str, sprintf(str, "BEGIN\r\n"), 1000);
		}
		else{}
		break;
	case 1:
	...
	case 4:
		if(value == 35){
			ok_flag = 1;
			//PROCESS OK
			HAL_UART_Transmit(&huart2, (void *)str, sprintf(str, "END\r\n"), 1000);
			state = 0;
		}
		else{state = 0;}
		break;
...
void main_fsm()
{
	if(buffer_flag == 1)
	{
		HAL_UART_Transmit(&huart2, (void *)str, sprintf(str, "BUFFER SIZE: = %d\r\n", index_buffer), 1000);
		main_command_parser();
//		command_parser_fsm();
		buffer_flag = 0;
	}
